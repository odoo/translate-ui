...
const { appTranslateFn, _t: translateFn } = translationModule;
...
function patched_t(source, ...substitutions) {
    ...
    const context = odoo.translationContext;
...
function patchedAppTranslateFn(source, context, ...substitutions) {
    const translation = appTranslateFn(source, context, ...substitutions);
    if (typeof translation !== "string") {
        return translation;
    }
    if (isTranslateModeEnabled()) {
...
translationModule.appTranslateFn = patchedAppTranslateFn;
